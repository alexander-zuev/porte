...
/** The requested page does not belong to the active conversation snapshot. */
export class StaleConversationCursorError extends TaggedError('StaleConversationCursorError')<{
  message: string
  classification: FailureClassification
}> {
  constructor() {
    super({ message: 'Start the conversation list again.', classification: 'terminal' })
  }
}
...
export class ConversationCatalog {
  ...
  continue(
    ...
    limit: number,
  ): ResultType<ListConversationsResult, StaleConversationCursorError> {
    const position = parseCursor(cursor)
    if (position === undefined || this.snapshot?.id !== position.snapshotId) {
      return Result.err(new StaleConversationCursorError())
    }
    return Result.ok(page(this.snapshot, position.start, limit))
  }
