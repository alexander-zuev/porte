...
export class FileCredentialStore implements CredentialStore {
  ...
  async read(): Promise<ResultType<StoredCredential | null, CredentialStoreError>> {
    let contents: string
    try {
      contents = await readFile(this.filePath, 'utf8')
    } catch (cause) {
      if (isMissing(cause)) return Result.ok(null)
      return Result.err(new CredentialStoreError({ cause }))
    }

    return Result.ok(parseCredential(contents))
  }
