...
function daemonTest(overrides?: { enabled?: boolean; credential?: StoredCredential | null }) {
  ...
  const deps: McpDaemonDeps = {
    ...
    lock,
    settings: { read: () => Promise.resolve({ enabled: overrides?.enabled ?? true, hook: false }) },
    state: {
    ...
    createRuntime: (signal: AbortSignal) => {
      ...
      return Promise.resolve({
        ...
        runtime: {
          run: (onStatus?: (status: RelayStatus) => void) => {
  ...
  return { clientTransport, deps, events, state: () => state }
}
