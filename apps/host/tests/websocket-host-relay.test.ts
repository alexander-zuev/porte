...
import {
  retryDelayMs,
  WebSocketHostEventPublisher,
} from '../src/infrastructure/relay/websocket-host-relay.ts'
...
describe('WebSocketHostEventPublisher', () => {
  it('routes a canonical event to its session audience', () => {
    const sent: string[] = []
    const publisher = new WebSocketHostEventPublisher((frame) => sent.push(frame))
