...
/** An account with no machine has no conversations, which is not a failure to read them. */
const NONE: ListConversationsResult = { conversations: [] }

/**
 * One page of the conversations on the account's machine.
 *
 * Answered from the relay's own replica, so a person pages through their
 * history whether or not the machine is awake. Owning no machine and owning a revoked
