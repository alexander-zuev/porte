...
import { EventIdSchema, MessageIdSchema, SessionIdSchema, TurnIdSchema } from './identity.ts'
...
/** Canonical turn, message, or reasoning event for one coding session. */
export const CodingSessionConversationEventSchema = z.intersection(
  z.object({ eventId: EventIdSchema, sessionId: SessionIdSchema }),
  conversationEventDataSchema,
)

/** Canonical turn, message, or reasoning event for one coding session. */
export type CodingSessionConversationEvent = z.infer<typeof CodingSessionConversationEventSchema>
