...
import { CodingSessionProgressEventSchema } from '../src/coding-session-progress-event.ts'

const base = { eventId: 'event-1', sessionId: 'session-1' }
const turnId = '0198b55e-49d6-7e0f-9917-b08777b451b9'

describe('CodingSessionProgressEventSchema', () => {
  it('parses a complete ordered plan', () => {
    const result = CodingSessionProgressEventSchema.safeParse({
      ...base,
      type: 'plan.updated',
      turnId,
  ...
  it('parses session usage with cumulative cost', () => {
    const result = CodingSessionProgressEventSchema.safeParse({
      ...base,
